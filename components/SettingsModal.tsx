...
interface SettingsModalProps {
  onClose: () => void;
  onSave: (settings: { enabled: boolean; days: number }) => void;
  currentSettings: { enabled: boolean; days: number };
}
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, onSave, currentSettings }) => {
  ...
  const handleSave = () => {
    onSave({ enabled, days: Math.max(0, days) }); // Ensure days is not negative
  };
        ...
        <div className="p-6">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <span className="text-gray-700">Enable expiry reminders</span>
            ...
            <div className={`transition-opacity duration-300 ${enabled ? 'opacity-100' : 'opacity-50'}`}>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  min="0"
                />
                <span className={enabled ? 'text-gray-700' : 'text-gray-500'}>days</span>
              </div>
            </div>
          </div>
